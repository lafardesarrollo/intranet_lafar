...
import { RequestAnulacionSolicitud,
...
@Component({
  selector: 'app-listoccostos',
  templateUrl: './listoccostos.component.html',
  styleUrls: ['./listoccostos.component.scss'],
})
export class ListoccostosComponent implements OnInit {
  ...
  onOpenOrden(codigo_orden: string): void {
    this.router.navigate(['/sc/orden/detailoccostos/' + codigo_orden]);
  ...
  onLoadListado() {
    this.openLoading();
    this.lorden = [];
    this.servOC.getListadoOrdenCostos(this.estado_autorizacion_subgerencia).subscribe(
      data => {
        this.closeLoading();
        if (data['length'] > 0) {
          this.lorden = data['body'];
