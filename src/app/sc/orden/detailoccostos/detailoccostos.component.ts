...
@Component({
  selector: 'app-detailoccostos',
  templateUrl: './detailoccostos.component.html',
  styleUrls: ['./detailoccostos.component.scss'],
})
export class DetailoccostosComponent implements OnInit {
  //#region 
  ...
  // Autorización de Orden de Compra
    ...
    descargarAdjunto(nombre_archivo: string){
      this.openLoading();
      let archivos_adjuntos: Array<string> = new Array<string>();

      this.servOC.obtenerArchivosAdjuntosOrdenCompra(this.codigo_orden, nombre_archivo).subscribe(
        (data: Array<string>) => {
          this.closeLoading();
          archivos_adjuntos = data;
          
          archivos_adjuntos.forEach(element => {
            let ruta: string = this.global.urlAPICORE + 'scfilesordencompra/' + btoa(unescape(encodeURIComponent(element)));
        ...
        (err: HttpErrorResponse) => {
          this.closeLoading();
          this.toast.show('No se descargo los archivos, intente nuevamente!', 1500, 'red');
        }
      );
    }
//#endregion
  ...
  onLoadAprobarOrdenCostos(codigo_orden: string, estado_autorizacion_costos: string): void {
    this.autorizarorden = new RequestAutorizacionSolicitud();
    this.autorizarorden.codigo_solicitud = codigo_orden;
    this.autorizarorden.autorizador = localStorage.getItem('username');
    this.autorizarorden.estado_autorizacion = estado_autorizacion_costos;
    this.onGuardarAutorizarOrdenCostos();
  }

  onGuardarAutorizarOrdenCostos() {
    this.openLoading();
    ...
    this.servOC.saveAutorizarOrdenCostos(this.autorizarorden).subscribe(
      data => {
        ...
        this.router.navigate(['/sc/orden/listcostos']);
