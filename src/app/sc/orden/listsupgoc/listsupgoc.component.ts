...
@Component({
  selector: 'app-listsupgoc',
  templateUrl: './listsupgoc.component.html',
  styleUrls: ['./listsupgoc.component.scss'],
})
export class ListsupgocComponent implements OnInit {
  ...
  term: string = '';
  estado_autorizacion_gerencia: string;
  ...
  onGetOrdenesCompra(): void {
    this.estado_autorizacion_gerencia = 'T';
    this.openLoading();
    this.servOC.getListadoOrdenXAutorizadorG(localStorage.getItem('username') + '_' + this.estado_autorizacion_gerencia).subscribe(
      data => {
        this.closeLoading();
        if (data['length'] > 0) {
  ...
  onOpenOrden(codigo_orden: string): void {
    this.router.navigate(['/sc/orden/detailocg/' + codigo_orden]);
  ...
  onGuardarAnularOrden() {
    ...
    this.servOC.saveAnularOrden(this.aorden).subscribe(
      data => {
        this.closeLoading();
        this.toast.show('Se anulo la orden!', 500, 'green');
        this.estado_autorizacion_gerencia = 'T';
        this.onLoadListado();
      },
      (err: HttpErrorResponse) => {
        this.closeLoading();
        this.toast.show('Ocurrio un error al anular la orden. Intente nuevamente!', 500, 'red');
        if (err.error instanceof Error) {
          // console.log('Ocurrio un error:', err.error.message);
        } else {
          // console.log(`El servidor respondio: ${err.status}, body was: ${err.error}`);
