...
@Component({
  selector: 'app-he-reporte-mensual',
  templateUrl: './he-reporte-mensual.component.html',
  styleUrls: ['./he-reporte-mensual.component.scss']
})
export class HeReporteMensualComponent implements OnInit {
    loading = $('#loading');  
    ...
    gestion: string;
    mes: string;
    gestiones: Array<string> = new Array<string>();
    meses: Array<string> = new Array<string>();
    ...
    ObtenerGestiones(){ // Obtiene los areas
        this.loading.fadeIn();
        this.heService.getGestiones().subscribe(
            resp => {
                this.gestiones = resp;
            }, error => {
                this.toast.show('Ocurrio un error al obtener las Gestiones', 1500);
            }
        ).add(()=> {
    ...
    ObtenerMeses(){ // Obtiene los areas
        this.loading.fadeIn();
        this.heService.getMeses(this.gestion).subscribe(
            resp => {
                this.meses = resp;
            }, error => {
                this.toast.show('Ocurrio un error al obtener las Gestiones', 1500);
