...
@Component({
  selector: 'app-he-nueva-solicitudgh',
  templateUrl: './he-nueva-solicitudgh.component.html',
  styleUrls: ['./he-nueva-solicitudgh.component.scss']
})
export class HeNuevaSolicitudghComponent implements OnInit {
    loading = $('#loading');  
    ...
    ObtenerEmpleadosDependientes(){
        ...
        this.loading.fadeIn();
        this.heService.getDependientesAll().subscribe(
            resp => {
                if(resp.status) {
  ...
  calcularCostoPorEmpleado(SalarioPorHora: number, CantidadHorasExtras: number, TipoHoraExtra: number, Turno: number): void
  {
    ...
    this.he.Costo = this.costoPorEmpleado; // Asignamos el Costo de las horas Extras
  }
